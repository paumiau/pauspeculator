...
class trade {
        ...
        public function __construct($key, $secret, $client_id, $exchange, $ecoin, $market)
        {
                if (isset($secret) && isset($key) && isset($client_id) && isset($exchange) && isset($ecoin) && isset($market))
                {
                        $this->key = $key;
                        $this->secret = $secret;
			...
			$this->market = $market;
                        if ( $exchange == 'bitstamp' ) {
                            ...
                            $this->oCryptsy=new Cryptsy($key, $secret);

                        } else {
                            die ("El Exchange debe ser bitstamp, btce o cryptsy. Pero has puesto ".$exchange."\r\n");
                        }
                        
                } else {
                        die("NO KEY/ SECRET/ CLIENT ID/ EXCHANGE/ ECOIN/ MARKET"."\r\n");
        	}
	}
        ...
        public function getTicker() {
            ...
            if ($exchange=='bitstamp') {
                ...
                $ticker =  $this->oBitstamp->bitstamp_query_public('https://www.bitstamp.net/api/ticker/');  //OJO: Poner la url en la class de bistamp
                $return_ticker['last'] = $ticker['last'];
                return $return_ticker;

            } else if ($exchange=='btce') {

                $ticker =  $this->oBtce->ticker($ecoin,$market);
                $return_ticker['last'] = $ticker['ticker']['last'];
                return $return_ticker;

           } else if ($exchange=='cryptsy') {

                $ticker =  $this->oCryptsy->ticker($ecoin, $market);
                $return_ticker['last'] = $ticker['return']['markets'][strtoupper($ecoin)]['lasttradeprice'];
                return $return_ticker;
